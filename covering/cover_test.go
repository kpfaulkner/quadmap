...
func quadKeyToWKT(t *testing.T, qk quadmap.QuadKey) string {
	env, err := qk.Envelope()
	require.NoError(t, err)
	return env.AsGeometry().AsText()
}

func mustGenerateQuadKeyIndexFromSlippy(x uint32, y uint32, zoomLevel byte) quadmap.QuadKey {
	qk, err := quadmap.GenerateQuadKeyIndexFromSlippy(x, y, zoomLevel)
	if err != nil {
		panic(err)
	}
	return qk
...
func TestSearchRanges(t *testing.T) {
	for _, tc := range []struct {
		name    string
		cells   []quadmap.QuadKey
		minZoom byte
		expect  []quadmap.QuadKeyRange
	}{
		{
			name:    "whole map",
			cells:   []quadmap.QuadKey{quadmap.QuadKey(0)},
			minZoom: 0,
			expect: []quadmap.QuadKeyRange{
		...
		{
			name:    "single tile",
			cells:   []quadmap.QuadKey{mustGenerateQuadKeyIndexFromSlippy(123, 456, 9)},
			minZoom: 0,
			expect: []quadmap.QuadKeyRange{
