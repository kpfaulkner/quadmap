...
func NewStorage(dbName string) (*Storage, error) {
	...
	db.MustExec(`create table if not exists details (id integer primary key, border varchar(500000),simple_border varchar(500000), tiletype integer, datetime integer, scale integer, identifier varchar(50), enabled bool)`)
...
func (s *Storage) CreatePartitionTableIfNotExist(tableName string) {
...
func (s *Storage) InsertDetails(details DetailsEntity) (int64, error) {
	res := s.db.MustExec(`INSERT INTO details ( border, simple_border, tiletype, datetime, enabled, scale, identifier) VALUES ($1,$2,$3,$4,$5,$6, $7);`, details.Border, details.SimpleBorder, details.TileType, details.DateTime, true, details.Scale, details.Identifier)
...
func (s *Storage) GetDetails(id int) (*DetailsEntity, error) {
	...
	s.db.Select(&entity, `SELECT id, border, simple_border, tiletype, datetime, scale, identifier FROM details WHERE enabled = true AND id = $1`, fmt.Sprintf("%d", id))
	return &entity, nil
}
...
func (s *Storage) SearchDetailsWithinQuadKey(qk quadtree.QuadKey, includeSimpleBorder bool) ([]DetailsEntity, error) {
	...
	if includeSimpleBorder {
		statement = fmt.Sprintf("select d.id,d.scale,d.identifier, d.simple_border from details d where d.id in (select distinct details_id from %s qm where  qm.quadkey >= $1 AND qm.quadkey <= $2 ) ;", tableName)
	} else {
		statement = fmt.Sprintf("select d.id,d.scale,d.identifier from details d where d.id in (select distinct details_id from %s qm where  qm.quadkey >= $1 AND qm.quadkey <= $2 ) ;", tableName)
	}
